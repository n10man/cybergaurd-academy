...
class OfficeScene extends Phaser.Scene {
  ...
  preload() {
    console.log('🎮 OfficeScene: Preloading MAP 1 (Office)...');
    
    // Load the Office map JSON
    this.load.tilemapTiledJSON('office_map', '/assets/maps/first_map.json');
  ...
  create() {
    console.log('🗺️ OfficeScene: Creating MAP 1');
    ...
    // Office-specific visual layers
    const visualLayerNames = ['Exterior most wall', 'White layer / barrier walls'];
    visualLayerNames.forEach(name => {
      const layer = this.map.createLayer(name, tilesets, 0, 0);
      if (layer) {
        if (name.toLowerCase().includes('top') || name.toLowerCase().includes('object')) {
          layer.setDepth(5);
        ...
        console.warn(`Layer not found in OfficeScene: ${name}`);
      }
    });
    ...
    if (this.collisionsLayer) {
      ...
      console.log('✅ OfficeScene: Collisions layer enabled');
    } else {
      console.error("❌ OfficeScene: 'collisions' layer not found");
    }
    ...
    console.log('✅ OfficeScene: Debug graphics ENABLED');
  }
  ...
  update() {
    ...
    if (nearestTarget) {
      ...
      if (this.spaceKey.isDown) {
        // Check if interaction object has targetScene property (e.g., door to ServerRoomScene)
        if (nearestInteractionData) {
          ...
          if (targetScene) {
            console.log(`🚪 OfficeScene: Transitioning to ${targetScene}`);
